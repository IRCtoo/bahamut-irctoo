...
/* local_rehash
 * Takes over m_rehash() once the targeted server is reached,
 * whether local or remote rehash is being performed.
 * - Holbrook
 */
int
...
/* m_rehash
 * parv[0] = sender prefix
 * parv[1] = server name or rehash option if no parv[2]
 * parv[2] = rehash option
 *
 * Changed m_rehash to handle determination of local vs remote rehash.
 * local_rehash() takes over the old m_rehash().
 * - Holbrook
 */
int
m_rehash(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	else
	{
		...
		local_rehash(cptr, sptr, parv[0], parv[2]);
	}
